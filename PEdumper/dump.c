...
DWORD va2pa2(_In_ DWORD address, _In_ IMAGE_SECTION_HEADER *section_header, _In_ IMAGE_FILE_HEADER *file_header, _In_ BYTE *mapped_file)
{
	DWORD i;
	DWORD offset;
	DWORD size;
	...
	for (i = 0; i < file_header->NumberOfSections; i++)
	{
		...
		if ((address >= section_header[i].VirtualAddress) && (address < section_header[i].VirtualAddress + size))
		{
			offset = address - section_header[i].VirtualAddress;
			return section_header[i].PointerToRawData + offset;
		}
	}
	return 0;
}
...
PE_STATUS dump_export_directory(_In_ IMAGE_EXPORT_DIRECTORY *export_directory, _In_ IMAGE_SECTION_HEADER *section_header, _In_ IMAGE_FILE_HEADER *file_header, _In_ LPVOID mapped_file)
{
	...
	//CHAR *name = (CHAR *)va2pa(export_directory->Name, section_header, file_header, mapped_file);
	//CHAR *name1 = export_directory->Name;
	//printf("\t%s -> Name\n", (CHAR *)(va2pa(export_directory->Name, section_header, file_header, mapped_file)));
	printf("\t%#x -> Name", export_directory->Name);
